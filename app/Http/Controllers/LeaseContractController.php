    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            $contract = LeaseContract::create([
                ...
                'rent_amount' => $totalContractAmount,
            ...
            // جلب حساب المستأجر وحساب المالك مرة واحدة خارج اللูป لتوفير الأداء (Performance)
            ...
            for ($i = 1; $i <= $installmentsCount; $i++) {
                ...
                $rentInstallment = RentInstallment::create([
                ...
                $dueDateString = $dueDate->format('Y-m-d'); // تحويل التاريخ لنص لتجنب أخطاء الـ Note

                // 4. تسجيل المعاملة المالية للمستأجر (مدين) لكل قسط
                if ($customerAccount) {
                    CreditTransactions::create([
                        'user_id' => Auth::id(),
                        'customer_id' => $customerAccount->id,
                        'recive_amount' => $installmentAmount,
                        'branchs_id' => Auth::user()->branchs_id ?? 1,
                        'pay_method' => $request->payment_type ?? 1,
                        'note' => 'عقد ايجار رقم : ' . $contract->id . ' قسط رقم ' . $rentInstallment->installment_number . ' يستحق تسديده يوم : ' . $dueDateString,
                        'currentblance' => 0,
                ...
                // 5. تسجيل المعاملة المالية للمالك (دائن) لكل قسط
                if ($onwerAccount) {
                    CreditTransactions::create([
                        'user_id' => Auth::id(),
